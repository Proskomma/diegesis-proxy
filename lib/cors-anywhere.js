...
function parseURL(req_url) {
  ...
  if (!['diegesis', 'ebible.org', 'git.door43.org'].includes(parsed.hostname)) {
    // Hostname whitelist
    return null;
  }
  return parsed;
...
function getHandler(options, proxy) {
  ...
  return function(req, res) {
    ...
    if (location.hostname === 'diegesis') {
      try {
        const staticContent = fse.readJSONSync(
            path.resolve(
                options.diegesisConfig.configPath,
                '..',
                options.diegesisConfig.staticDir,
                location.pathname.substring(1) // Remove leading /
            )
        )
        res.setHeader('content-type', 'application/json; charset=utf-8');
        res.setHeader('access-control-allow-origin', '*');
        res.end(JSON.stringify(staticContent));
        return;
      } catch (err) {
        res.writeHead(404, 'Not Found (Static)', cors_headers);
